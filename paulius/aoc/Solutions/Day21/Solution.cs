...
file class Solution : ISolution<Input>
{
    ...
    {
        return GetDistance(grid, grid.First(kv => kv.Value is 'S').Key, 6);
    }

    public object Part2(Input grid)
    {
        Test();
        return GetDistance2(grid, grid.First(kv => kv.Value is 'S').Key, 26501365);
    }


    public delegate long? CostFunction(Input map, Coord2D from, Coord2D to);
    public static int GetDistance(Input map, Coord2D start, int steps)
    {
    ...
    public static int GetDistance2(Input map, Coord2D start, int steps)
    {
        ...
        for (int i = 0; i < steps; i++)
        {
            q = (from current in q
                    from n in GetNear4(current)
                    where map[Pin(n, maxR, maxC)] is '.' or 'S'
                    select n).ToHashSet();
        }
